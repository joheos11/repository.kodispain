...
def toggleAdult():
	...
	if len(found) > 0:
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to view a list of the addons that where %s?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR white]Ver Lista[/COLOR][/B]", nolabel="[B][COLOR white]Cancelar[/COLOR][/B]"):
...
def toggleAddon(id, value, over=None):
	...
	if 'error' in response and over == None:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error %s [COLOR %s]%s[/COLOR]" % (COLOR2, v, COLOR1 , id), "Comprueba que la lista de addons esta actualizada e intentalo de nuevo.[/COLOR]")
		forceUpdate()
...
def createQR():
	url = getKeyboard('', "%s: Escribe la URL para el Codigo QR." % ADDONTITLE)
	if url == "": LogNotify("[COLOR %s]Crear QR[/COLOR]" % COLOR1, '[COLOR %s]Crear Codigo QR Cancelado![/COLOR]' % COLOR2); return
	if not url.startswith('http://') and not url.startswith('https://'): LogNotify("[COLOR %s]Create QR[/COLOR]" % COLOR1, '[COLOR %s]No es una URL correcta![/COLOR]' % COLOR2); return
	if url == 'http://' or url == 'https://': LogNotify("[COLOR %s]Crear QR[/COLOR]" % COLOR1, '[COLOR %s]No es una URL correcta![/COLOR]' % COLOR2); return
	working = workingURL(url)
	if not working == True:
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Parece que la URL no funciona, te gustaria crearla de todos modos?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR red]Crear[/COLOR][/B]", nolabel="[B][COLOR springgreen]Cancelar[/COLOR][/B]"):
			return
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Localizacion de Copia de Seguridad: Vacia[/COLOR]" % (COLOR2))
		return
	...
	selected = DIALOG.select("%s: Selecciona los elementos a eliminar de 'MyBuilds'." % ADDONTITLE, list)

	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Limpieza cancelada![/COLOR]" % COLOR2)
	elif selected == 0:
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres limpiar todos los elementos de la carpeta 'My_Builds'?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR white]Aceptar[/COLOR][/B]", nolabel="[B][COLOR white]Cancelar[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres eliminar [COLOR %s]%s[/COLOR] de la carpeta 'My_Builds'?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR springgreen]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
...
def convertSpecial(url, over=False):
	...
	DP.create(ADDONTITLE, "[COLOR %s]Cambiando Directorios de Local a Especial" % COLOR2, "", "Please Wait[/COLOR]")
...
def asciiCheck(use=None, over=False):
	if use == None:
		source = DIALOG.browse(3, '[COLOR %s]Selecciona la carpeta que quieres comprobar[/COLOR]' % COLOR2, 'files', '', False, False, HOME)
		if over == True:
			yes = 1
		else:
			yes = DIALOG.yesno(ADDONTITLE,'[COLOR %s]Quieres [COLOR %s]Borrar[/COLOR] todos los nombres de archivos con caracteres especiales o prefieres [COLOR %s]comprobar y ver[/COLOR] los resultados en el log?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLOR white]Borrar[/COLOR][/B]', nolabel='[B][COLOR white]Comprobar[/COLOR][/B]')
	else:
		source = use
		yes = 1

	if source == "":
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Comprobacion ASCII: Cancelada[/COLOR]" % COLOR2)
		return
	...
	DP.create(ADDONTITLE, 'Por favor, espera...')
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2,"[COLOR %s]Comprobando caracteres especiales" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), 'Por favor, espera[/COLOR]')
			try:
				file.encode('ascii')
			except UnicodeEncodeError:
				wiz.log("[ASCII Check] Caracter especial encontrado en archivo: {0}".format(item.filename))
				...
				if yes:
					try:
						...
						afiles.write('\n')
						f1 += 1
						log("[ASCII Check] Archivo Eliminado: %s " % badfile, xbmc.LOGERROR)
					except:
						for chunk in chunks(badfile, 75):
							afails.write(chunk+'\n')
						afails.write('\n')
					...
					afiles.write('\n')
					f1 += 1
					log("[ASCII Check] Archivo Encontrado: %s " % badfile, xbmc.LOGERROR)
				pass
		if DP.iscanceled():
			DP.close()
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Comprobacion Ascii Cancelada[/COLOR]" % COLOR2)
			sys.exit()
	...
	if total > 0:
		...
		if yes:
			if use:
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Comprobacion Ascii: %s Eliminado / %s Fallido.[/COLOR]" % (COLOR2, f1, f2))
			else:
				TextBox(ADDONTITLE, "[COLOR yellow][B]%s Archivos Eliminados:[/B][/COLOR]\n %s\n\n[COLOR yellow][B]%s Archivos fallidos:[B][/COLOR]\n %s" % (f1, msg, f2, msg2))
		else:
			TextBox(ADDONTITLE, "[COLOR yellow][B]%s Archivos encontrados:[/B][/COLOR]\n %s" % (f1, msg))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Comprobacion Ascii: Nada Encontrado.[/COLOR]" % COLOR2)
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Eliminar Paquetes" % COLOR2, "[COLOR %s]%s[/COLOR] archivos encontrados. Ocupan [COLOR %s]%s[/COLOR]" % (COLOR1, str(file_count), COLOR1, size), "Quieres Borrarlos?[/COLOR]", yeslabel='[B][COLOR white]Aceptar[/COLOR][/B]',nolabel='[B][COLOR white]Cancelar[/COLOR][/B]')
					if yes:
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Paquetes eliminados correctamente: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]No se encontraron Paquetes que Eliminar[/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Error al eliminar Paquetes[/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]No se encontraron Paquetes que Eliminar[/COLOR]' % COLOR2)
...
def checkSources():
	...
	if len(match) > 0:
		...
		DP.create(ADDONTITLE, "[COLOR %s]Comprobando Fuentes[/COLOR]" % COLOR2)
		...
		if len(bad) > 0:
			choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] Fuente(s) no funcionan" % (COLOR1, len(bad), COLOR2),"Deseas eliminar TODAS o elegir una a una?[/COLOR]", yeslabel="[B][COLOR white]Eliminar Todas[/COLOR][/B]", nolabel="[B][COLOR red]Elegir una a una[/COLOR][/B]")
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad:
					log("%s sources: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] no funciona" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR white]Eliminar Fuente[/COLOR][/B]", nolabel="[B][COLOR red]Mantener Fuente[/COLOR][/B]"):
						remove.append([name, path, sharing, working])
						log("Eliminando Fuente %s" % name, xbmc.LOGNOTICE)
					else: log("La Fuente %s no fue eliminada" % name, xbmc.LOGNOTICE)
			if len(remove) > 0:
				...
				removed = len(remove)
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Se ha completado la comprobacion de las fuentes" % COLOR2, "Funcionando: [COLOR %s]%s[/COLOR] | Conservas: [COLOR %s]%s[/COLOR] | Eliminas: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
			else: log("No hay nada que eliminar.", xbmc.LOGNOTICE)
		else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todas la Fuentes Funcionan[/COLOR]" % COLOR2)
...
def checkRepos():
	...
	if len(badrepos) > 0:
		msg  = "[COLOR %s]A continuacion hay una lista de Repositorios que no funcionan en este momento. Esto no significa que ya no vayan a funcionar. Comprueba que el fallo se prolonga en el tiempo antes de eliminar un repositorio.[/COLOR][CR][CR][COLOR %s]" % (COLOR2, COLOR1)
		msg += '[CR]'.join(badrepos)
		msg += '[/COLOR]'
		TextBox("%s: Repositorio(s) no responde(n)" % ADDONTITLE, msg)
	else:
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('Forzar Cierre de Kodi', '[COLOR %s]Estas a punto de cerrar Kodi' % COLOR2, 'Te gustaria continuar?[/COLOR]', yeslabel='[B][COLOR white]Continuar[/COLOR][/B]',nolabel='[B][COLOR white]Cancelar[/COLOR][/B]')
	if choice == 1:
