...
def loginIt(do, who):
	...
	if who == 'all':
		for log in ORDER:
			...
			else: wiz.log('[Info Login] %s(%s) no esta instalado' % (LOGINID[log]['name'],LOGINID[log]['plugin']), xbmc.LOGERROR)
		wiz.setS('loginlastsave', str(THREEDAYS))
	else:
		if LOGINID[who]:
			if os.path.exists(LOGINID[who]['path']):
				updateLogin(do, who)
		else: wiz.log('[Info Login] Entrada invalida: %s' % who, xbmc.LOGERROR)
...
def clearSaved(who, over=False):
	if who == 'all':
		...
		if os.path.exists(file):
			os.remove(file)
			wiz.LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, LOGINID[who]['name']), '[COLOR %s]Info Login: Eliminado![/COLOR]' % COLOR2, 2000, LOGINID[who]['icon'])
	...
	if do == 'update':
		if not user == '':
			try:
				...
				user = addonid.getSetting(default)
				wiz.setS(saved, user)
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, name),'[COLOR %s]Login Data: Guardado![/COLOR]' % COLOR2, 2000, icon)
			except Exception, e:
				wiz.log("[Login Data] No se puede actualizar %s (%s)" % (who, str(e)), xbmc.LOGERROR)
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, name),'[COLOR %s]Login Data: No Registrado![/COLOR]' % COLOR2, 2000, icon)
	elif do == 'restore':
		if os.path.exists(file):
			...
			try:
				...
				user = addonid.getSetting(default)
				wiz.setS(saved, user)
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, name), '[COLOR %s]Login: Restaurado![/COLOR]' % COLOR2, 2000, icon)
			except Exception, e:
				wiz.log("[Info Login] No se puede actualizar %s (%s)" % (who, str(e)), xbmc.LOGERROR)
			...
			try:
				...
				for line in lines:
					...
					if len(match) == 0: f.write(line)
						...
						else: wiz.log('Eliminando Linea: %s' % line, xbmc.LOGNOTICE)
				f.close()
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, name),'[COLOR %s]Addon Data: Limpiado![/COLOR]' % COLOR2, 2000, icon)
			except Exception, e:
				wiz.log("[Trakt Data] No es posible limpiar el Addon %s (%s)" % (who, str(e)), xbmc.LOGERROR)
	wiz.refresh()
	...
	if who == 'all':
		...
		if os.path.exists(LOGINID[who]['path']):
			...
			if u == None or u == '': return
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres guardar el [COLOR %s]Info Login[/COLOR] para [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "Addon: [COLOR springgreen][B]%s[/B][/COLOR]" % u, "Saved:[/COLOR] [COLOR red][B]%s[/B][/COLOR]" % su if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Ninguno[/B][/COLOR]', yeslabel="[B][COLOR springgreen]Guardar Datos[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
					loginIt('update', who)
...
def importlist(who):
	if who == 'all':
		...
		if os.path.exists(LOGINID[who]['file']):
			...
			if len(m) > 0:
				if not m[0] == su:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres importar el [COLOR %s]Info Login[/COLOR] para [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "Archivo: [COLOR springgreen][B]%s[/B][/COLOR]" % m[0], "Guardado:[/COLOR] [COLOR red][B]%s[/B][/COLOR]" % su if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Ninguno[/B][/COLOR]', yeslabel="[B][COLOR springgreen]Guardar Datos[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
						wiz.setS(sa, m[0])
						wiz.log('[Import Data] %s: %s' % (who, str(m)), xbmc.LOGNOTICE)
					else: wiz.log('[Import Data] Importar Cancelado(%s): %s' % (who, str(m)), xbmc.LOGNOTICE)
				else: wiz.log('[Import Data] Entrada Duplicada(%s): %s' % (who, str(m)), xbmc.LOGNOTICE)
			else: wiz.log('[Import Data] No hay Coincidencias(%s): %s' % (who, str(m)), xbmc.LOGNOTICE)
