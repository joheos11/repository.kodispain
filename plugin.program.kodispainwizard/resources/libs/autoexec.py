...
def main():
	class enableAll():
		def __init__(self):
			...
			if not os.path.exists(os.path.join(self.databasepath, self.dbfilename)):
				xbmcgui.Dialog().notification("AutoExec.py", "No existe archivo Addons27.db")
				...
				if os.path.exists(addonxml):
					...
					try:
						...
						try:
							...
							self.log("No se puede habilitar: %s" % folder, xbmc.LOGERROR)
		...
		def log(self, msg, level=xbmc.LOGNOTICE):
			try:
				...
				try: xbmc.log('[AutoExec.py] Error de registro: %s' % (e), xbmc.LOGERROR)
				except: pass
		...
		def swapUS(self):
			...
			self.log("Obtener Ajustes de Fuentes Desconocidas: %s" % str(response), xbmc.LOGDEBUG)
			if 'false' in response:
				...
				response = xbmc.executeJSONRPC(query)
				xbmcgui.Dialog().notification("AutoExec.py", "Fuentes desconocidas: Habilitado")
				self.log("Obtener Ajustes de Fuentes Desconocidas: %s" % str(response), xbmc.LOGDEBUG)
		...
		def addonDatabase(self, addon=None, state=1, array=False):
			...
			if os.path.exists(self.dbfilename):
				try:
					...
					self.log("Error de conexion DB: %s" % str(e), xbmc.LOGERROR)
					return False
			else: return False
			try:
				if array == False:
					textexe.execute('INSERTAR o IGNORAR en instalado (addonID , enabled, installDate) VALUES (?,?,?)', (addon, state, installedtime,))
					textexe.execute('ACTUALIZACION instalada SET habilitado = ? WHERE addonID = ? ', (state, addon,))
				else:
					for item in addon:
						textexe.execute('INSERTAR o IGNORAR en instalado (addonID , enabled, installDate) VALUES (?,?,?)', (item, state, installedtime,))
						textexe.execute('ACTUALIZACION instalada SET habilitado = ? WHERE addonID = ? ', (state, item,))
				textdb.commit()
				textexe.close()
			except Exception, e:
	...
	try:
		...
		firstRun = enableAll()
		xbmcgui.Dialog().notification("AutoExec.py", "Todos los addons activados")
