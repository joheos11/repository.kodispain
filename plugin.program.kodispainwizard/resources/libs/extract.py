...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	try:
		...
		errors += 1; error += '%s\n' % e
		wiz.log('Error Comprobando Zip: %s' % str(e), xbmc.LOGERROR)
	...
	for item in zin.infolist():
		try:
			...
			wiz.log("[ASCII Check] Caracter ASCII encontrado en el archivo: {0}".format(item.filename))
			continue
		...
		line2 += '[COLOR %s][B]Total:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR]' % (COLOR2, COLOR1, wiz.convertSize(size), zipsize)
		...
		if skip == True: wiz.log("Omitiendo: %s" % item.filename, xbmc.LOGNOTICE)
		else:
			try:
