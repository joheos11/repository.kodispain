...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Configurador de Ajustes Avanzados[/COLOR]' % (COLOR2)
			...
			header = '[COLOR %s]Cache de Video[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]Numero de bytes utilizados para almacenar buffer en la memoria.  Cuando se ajusta a [COLOR %s]0[/COLOR] el cache se escribira en el disco en lugar de la RAM.  Nota: Para la cantidad de memoria establecida aqui, Kodi requerira 3 veces la cantidad de RAM libre. Establecer esto demasiado alto podria hacer que Kodi se bloquee si no consigue suficiente RAM(1/3 de Memoria Libre: [COLOR %s]%s[/COLOR])[/COLOR]" % (COLOR2, COLOR1, COLOR1, freeMemory)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Actualmente no hay ninguna buil instalada de %s.\n\nSelecciona 'Instalar' para instalar una o 'Cerrar' para no ver este mensaje otra vez.\n\nGracias por elegir %s." % (ADDONTITLE, ADDONTITLE)
