...
def index():
    ...
    errorsfound = str(errors) + ' Error(es) encontrado (s)' if errors > 0 else 'Nada encontrado'
    ...
    else: addDir('Ninguna', 'builds', themeit=THEME4)
    ...
    addDir ('Guardar Datos', 'savedata', icon=ICONSAVE,     themeit=THEME1)
    if HIDECONTACT == 'No': addFile('Contacto' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    addFile('Compartir Archivo kodi.log', 'uploadlog',       icon=ICONMAINT, themeit=THEME1)
    addFile('Ver Errores en Log: %s' % (errorsfound), 'viewerrorlog', icon=ICONMAINT, themeit=THEME1)
    if errors > 0: addFile('Ver Ultimo Error En Log', 'viewerrorlast', icon=ICONMAINT, themeit=THEME1)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    addFile('Ajustes', 'settings', icon=ICONSETTINGS, themeit=THEME1)
    addFile('Forzar Actualizacion de Archivos de Texto', 'forcetext', icon=ICONMAINT, themeit=THEME1)
    if DEVELOPER == 'true': addDir('Menu de Desarrollador', 'developer', icon=ICON, themeit=THEME1)
...
def buildMenu():
    ...
    if bf == False:
        ...
        addDir ('Guarda Datos'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
        addFile('Url para archivo txt no valido', '', icon=ICONBUILDS, themeit=THEME3)
    ...
    if len(match) >= 1:
        if SEPERATE == 'true':
            ...
            if count15 > 0:
                ...
                addFile('[B]%s Isengard y versiones anteriores(%s)[/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME3)
                if SHOW15 == 'true':
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
        ...
        if adultcount > 0:
            addFile('Actualmente solo hay versiones para adultos.', '', icon=ICONBUILDS, themeit=THEME3)
            addFile('Habilitar mostrar adultos en la configuracio-n del Addon > Misc', '', icon=ICONBUILDS, themeit=THEME3)
        else:
            addFile('Actualmente no se ofrecen builds desde %s' % ADDONTITLE, '', icon=ICONBUILDS, themeit=THEME3)
    else: addFile('Archivos de texto para builds no formarteado correctamente.', '', icon=ICONBUILDS, themeit=THEME3)
    setView('files', 'viewType')
...
def viewBuild(name):
    ...
    if bf == False:
        WORKINGURL = wiz.workingURL(BUILDFILE)
        addFile('Url para el archivo txt no valido', '', themeit=THEME3)
    ...
    if wiz.checkBuild(name, 'version') == False:
        ...
        addFile('%s no se encontro en la lista de compilaciones.' % name, '', themeit=THEME3)
        return
    ...
    for version, url, minor, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
        addDir ('Guardar Datos',       'savedata', icon=ICONSAVE,     themeit=THEME3)
        addFile('Informacion de Build',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
        if not preview == "http://": addFile('Ver Video Instalacion', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
        ...
        if warning == True:
            addFile('[I]Build creada para la version de kodi %s(installed: %s)[/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
        addFile(wiz.sep('INSTALAR'), '', fanart=fanart, icon=icon, themeit=THEME3)
        addFile('Instalacion nueva'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
        addFile('Instalacion normal', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def viewThirdList(number):
    ...
    if not work == True:
        addFile('Url no valida para el archivo txt', '', icon=ICONBUILDS, themeit=THEME3)
        addFile('%s' % WORKINGURL, '', icon=ICONBUILDS, themeit=THEME3)
    else:
...
def editThirdParty(number):
    name  = eval('THIRD%sNAME' % number)
    url   = eval('THIRD%sURL' % number)
    name2 = wiz.getKeyboard(name, 'Escribe el Nombre del Wizard')
    url2  = wiz.getKeyboard(url, 'Escribe la URL del Wizard')
...
def apkScraper(name=""):
    if name == 'kodi':
        ...
        addFile("Apk\'s Oficiales de Kodi", themeit=THEME1)
        rc = False
        ...
        for url, name, size, date in match2:
            ...
            try:
                ...
                wiz.log("Error en: %s" % name)
        if x == 0: addFile("Error Kodi Scraper Esta Actualmente Caido.")

def apkMenu(name=None, url=None):
    if url == None:
        addDir ('Apk\'s Oficiales de Kodi', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    if not APKFILE == 'http://':
        if url == None:
        ...
        if not TEMPAPKFILE == False:
            ...
            if len(match) > 0:
                ...
                for aname, section, url, icon, fanart, adult, description in match:
                    ...
                    if x == 0:
                        addFile("Todavia No hay addons incluidos en este menu!", '', themeit=THEME2)
            else: wiz.log("[APK Menu] ERROR: Formato No Valido.", xbmc.LOGERROR)
        else:
            wiz.log("[APK Menu] ERROR: URL para lista apk no funciona.", xbmc.LOGERROR)
            addFile('La Url para el archivo txt no es valida', '', themeit=THEME3)
            addFile('%s' % APKWORKING, '', themeit=THEME3)
        return
    else: wiz.log("[APK Menu] No se ha incluido una lista de APKs.")
    setView('files', 'viewType')
    ...
    if not ADDONFILE == 'http://':
        ...
        if not TEMPADDONFILE == False:
            ...
            if len(match) > 0:
                ...
                addFile('Archivo de Texto no formateado correctamente!', '', themeit=THEME3)
                wiz.log("[Addon Menu] ERROR: El Formato No es Correcto.")
        else:
            wiz.log("[Addon Menu] ERROR: La URL para lista de Addons no funciona.")
            addFile('La Url para el archivo txt no es correcta', '', themeit=THEME3)
            addFile('%s' % ADDONWORKING, '', themeit=THEME3)
    else: wiz.log("[Addon Menu] No se ha incluido una lista de Addons.")
    setView('files', 'viewType')

def packInstaller(name, url):
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Intalador de Addons[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Url Zip Incorrecta![/COLOR]' % (COLOR1, name, COLOR2)); return
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    DP.create(ADDONTITLE,'[COLOR %s][B]Descargando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Espera Por favor[/COLOR]' % COLOR2)
    urlsplits = url.split('/')
    ...
    DP.update(0, title,'', '[COLOR %s]Espera Por favor[/COLOR]' % COLOR2)
...
def skinInstaller(name, url):
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Instalador de Addons[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Archivo Incorrecto![/COLOR]' % (COLOR1, name, COLOR2)); return
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    DP.create(ADDONTITLE,'[COLOR %s][B]Descargando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Espera Por favor[/COLOR]' % COLOR2)
    urlsplits = url.split('/')
    lib = xbmc.makeLegalFilename(os.path.join(PACKAGES, urlsplits[-1]))
    ...
    for item in installed:
        if item.startswith('skin.') == True and not item == 'skin.shortcuts':
            ...
            wiz.swapSkins(item, 'Instalador de Skins')
    wiz.refresh()
...
def addonInstaller(plugin, url):
    if not ADDONFILE == 'http://':
        ...
        if ADDONWORKING == True:
            ...
            if len(match) > 0:
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        do        = ['Abrir Addon', 'Borrar Addon']
                        selected = DIALOG.select("[COLOR %s]Addon Instalado, que te gustaria hacer ahora?[/COLOR]" % COLOR2, do)
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar los datos de:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR springgreen]Aceptar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                    ...
                    if not repository.lower() == 'none' and not os.path.exists(repo):
                        ...
                        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Te gustaria instalar el repositorio para [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR springgreen]Aceptar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                            ...
                            if len(ver) > 0:
                                ...
                                wiz.log("[Addon Installer] Repositorio no instalado: No se puede encontrar la url! (%s)" % repository)
                        else: wiz.log("[Addon Installer] Repositorio para %s no instalado: %s" % (plugin, repository))
                    elif repository.lower() == 'none':
                        wiz.log("No hay repositorio, instalando addon")
                        pluginid = plugin
                        zipurl = url
                        ...
                        wiz.log("Repositorio instalado, instalando addon")
            ...
            else: wiz.log("[Addon Installer] Formato Incorrecto")
        else: wiz.log("[Addon Installer] Archivo de Text: %s" % ADDONWORKING)
    else: wiz.log("[Addon Installer] No Activado.")
...
def installed(addon):
    ...
    if os.path.exists(url):
        try:
            ...
            wiz.LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, name[0]), '[COLOR %s]Addon Activado[/COLOR]' % COLOR2, '2000', icon)
        except: pass
...
def youtubeMenu(name=None, url=None):
    if not YOUTUBEFILE == 'http://':
        ...
        if not TEMPYOUTUBEFILE == False:
            ...
            else: wiz.log("[Menu YouTube] ERROR: Formato Incorrecto.")
        else:
            wiz.log("[Menu YouTube] ERROR: La URL para la lista YouTube no funciona.")
            addFile('La URL del archivo txt no es correcta', '', themeit=THEME3)
            addFile('%s' % YOUTUBEWORKING, '', themeit=THEME3)
    else: wiz.log("[Menu YouTube] Lista YouTube no cargada.")
    setView('files', 'viewType')

def maintMenu(view=None):
    on = '[B][COLOR springgreen]Activado[/COLOR][/B]'; off = '[B][COLOR red]Desactivado[/COLOR][/B]'
    ...
    errors = int(errorChecking(count=True))
    errorsfound = str(errors) + ' Error(es) Encontrado (s)' if errors > 0 else 'Nada encontrado'
    wizlogsize = ': [COLOR red]No encontrado[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR springgreen]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
    if includeall == 'true':
    ...
    feq        = ['Siempre', 'Diaria', '3 Dias', 'Semanal']
    ...
    if view == "clean" or SHOWMAINT == 'true':
        ...
        addFile('Borrar Cache: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache),       'clearcache',      icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Paquetes: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack),     'clearpackages',   icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Miniaturas: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),  'clearthumb',      icon=ICONMAINT, themeit=THEME3)
        if os.path.exists(ARCHIVE_CACHE): addFile('Borrar Archive_Cache: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(archive), 'cleararchive',    icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Miniaturas antiguas', 'oldThumbs',      icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Registros de fallos',               'clearcrash',      icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Bases de Datos',                'purgedb',         icon=ICONMAINT, themeit=THEME3)
        addFile('[B]BORRAR TODOS LOS DATOS[/B]',                    'freshstart',      icon=ICONMAINT, themeit=THEME3)
    addDir ('[B]Mantenimiento de Addons[/B]',       'maint', 'addon',  icon=ICONMAINT, themeit=THEME1)
    if view == "addon" or SHOWMAINT == 'true':
        addFile('Eliminar Addons',                  'removeaddons',    icon=ICONMAINT, themeit=THEME3)
        addDir ('Eliminar Datos de Addons',              'removeaddondata', icon=ICONMAINT, themeit=THEME3)
        addDir ('Activar/Desactivar Addons',          'enableaddons',    icon=ICONMAINT, themeit=THEME3)
        addFile('Activar/Desactivar Addons de Adultos',    'toggleadult',     icon=ICONMAINT, themeit=THEME3)
        addFile('Forzar Actualizacion de Addons',            'forceupdate',     icon=ICONMAINT, themeit=THEME3)
        addFile('Ocultar password en la entrada del teclado',   'hidepassword',   icon=ICONMAINT, themeit=THEME3)
        addFile('Mostrar password en la entrada del teclado', 'unhidepassword', icon=ICONMAINT, themeit=THEME3)
    addDir ('[B]Otras Tareas de Mantenimiento[/B]'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME1)
    if view == "misc" or SHOWMAINT == 'true':
        addFile('Arreglar Kodi 17',                    'kodi17fix',       icon=ICONMAINT, themeit=THEME3)
        addDir ('Prueba de velocidad',                     'speedtest',       icon=ICONMAINT, themeit=THEME3)
        addFile('Habilitar fuentes desconocidas',         'unknownsources',  icon=ICONMAINT, themeit=THEME3)
        ...
        addFile('Cargar archivo de registro', 'uploadlog',       icon=ICONMAINT, themeit=THEME3)
        addFile('Ver errores en el registro: %s' % (errorsfound), 'viewerrorlog', icon=ICONMAINT, themeit=THEME3)
        if errors > 0: addFile('Ver Ultimo Error del Log', 'viewerrorlast', icon=ICONMAINT, themeit=THEME3)
        addFile('Ver Kodi.Log',                  'viewlog',         icon=ICONMAINT, themeit=THEME3)
        addFile('Ver Wizard.Log',           'viewwizlog',      icon=ICONMAINT, themeit=THEME3)
        addFile('Borrar Wizard.Log%s' % wizlogsize,'clearwizlog',     icon=ICONMAINT, themeit=THEME3)
    addDir ('[B]Copias de Seguridad[/B]'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
    if view == "backup" or SHOWMAINT == 'true':
        addFile('Limpiar la carpeta de Copias de Seguridad',        'clearbackup',     icon=ICONMAINT,   themeit=THEME3)
        addFile('Ubicacion de Copias de Seguridad: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Mantenimiento', icon=ICONMAINT, themeit=THEME3)
        #addFile('Extract a ZipFile',              'extractazip',     icon=ICONMAINT,   themeit=THEME3)
        addFile('Guardar Build',               'backupbuild',     icon=ICONMAINT,   themeit=THEME3)
        addFile('Guardar GuiFix',              'backupgui',       icon=ICONMAINT,   themeit=THEME3)
        addFile('Guardar Tema',               'backuptheme',     icon=ICONMAINT,   themeit=THEME3)
        addFile('Guardar Addons',          'backupaddonpack', icon=ICONMAINT,   themeit=THEME3)
        addFile('Guardar Addon_data',          'backupaddon',     icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar Build Local',         'restorezip',      icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar GuiFix Local',        'restoregui',      icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar Addon_data Local',    'restoreaddon',    icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar External Build',      'restoreextzip',   icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar External GuiFix',     'restoreextgui',   icon=ICONMAINT,   themeit=THEME3)
        addFile('Recuperar External Addon_data', 'restoreextaddon', icon=ICONMAINT,   themeit=THEME3)
    addDir ('[B]Ajustes/Correcciones de Sistema[/B]',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME1)
    if view == "tweaks" or SHOWMAINT == 'true':
        if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
            addDir ('Ajustes avanzados',            'advancedsetting',  icon=ICONMAINT, themeit=THEME3)
        else:
            if os.path.exists(ADVANCED):
                addFile('Ver AdvancedSettings.xml actual',   'currentsettings', icon=ICONMAINT, themeit=THEME3)
                addFile('Eliminar AdvancedSettings.xml actual', 'removeadvanced',  icon=ICONMAINT, themeit=THEME3)
            addFile('Configuracion rapida de AdvancedSettings.xml',    'autoadvanced',    icon=ICONMAINT, themeit=THEME3)
        addFile('Comprobar Fuentes',  'checksources',    icon=ICONMAINT, themeit=THEME3)
        addFile('Comprobar Repositorios',   'checkrepos',      icon=ICONMAINT, themeit=THEME3)
        addFile('Arreglar Addons No Actualizando',        'fixaddonupdate',  icon=ICONMAINT, themeit=THEME3)
        addFile('Eliminar nombres de archivo que no sean Ascii',     'asciicheck',      icon=ICONMAINT, themeit=THEME3)
        addFile('Convertir Rutas a special',       'convertpath',     icon=ICONMAINT, themeit=THEME3)
        addDir ('Informacion del sistema',             'systeminfo',      icon=ICONMAINT, themeit=THEME3)
    addFile('Mostrar Todo: %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
    addDir ('[I]<< Volver al Menu Principal[/I]', icon=ICONMAINT, themeit=THEME2)
    addFile('Incluye tus propios Wizards: %s' % thirdparty.replace('true',on).replace('false',off) ,'togglesetting', 'enable3rd', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
    if thirdparty == 'true':
        ...
        addFile('Incluir Wizard 1: [COLOR %s]%s[/COLOR]' % (COLOR2, first), 'editthird', '1', icon=ICONMAINT, themeit=THEME3)
        addFile('Incluir Wizard 2: [COLOR %s]%s[/COLOR]' % (COLOR2, secon), 'editthird', '2', icon=ICONMAINT, themeit=THEME3)
        addFile('Incluir Wizard 3: [COLOR %s]%s[/COLOR]' % (COLOR2, third), 'editthird', '3', icon=ICONMAINT, themeit=THEME3)
    addFile('Limpieza automatica', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
    ...
    if autoclean == 'true':
        addFile('--- Frecuencia de Limpieza automatica: [B][COLOR springgreen]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
        addFile('--- Borrar cache al inicio: %s' % cache.replace('true',on).replace('false',off), 'togglesetting', 'clearcache', icon=ICONMAINT, themeit=THEME3)
        addFile('--- Borrar paquetes al inicio: %s' % packages.replace('true',on).replace('false',off), 'togglesetting', 'clearpackages', icon=ICONMAINT, themeit=THEME3)
        addFile('--- Limpiar miniaturas antiguas al inicio: %s' % thumbs.replace('true',on).replace('false',off), 'togglesetting', 'clearthumbs', icon=ICONMAINT, themeit=THEME3)
    addFile('Borrar cache de video', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
    addFile('Incluir cache de video en Borrar cache: %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME3)
    if includevid == 'true':
...
def net_tools(view=None):
    addDir ('Prueba de Velocidad' ,'speedtestM', icon=ICONAPK, themeit=THEME1)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    addDir ('Ver Direcciones IP & MAC',        'viewIP',    icon=ICONMAINT, themeit=THEME1)
    setView('files', 'viewType')
...
def viewIP():
    ...
    addFile('[COLOR %s]Estado:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, state), '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Pais:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, country), '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Proveedor:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, isp), '', icon=ICONMAINT, themeit=THEME2)
    setView('files', 'viewType')

def speedTest():
    addFile('Iniciar Prueba de Velocidad',             'runspeedtest',      icon=ICONMAINT, themeit=THEME3)
    if os.path.exists(SPEEDTESTFOLD):
        ...
        if len(speedimg) > 0:
            addFile('Borrar Resultados',          'clearspeedtest',    icon=ICONMAINT, themeit=THEME3)
            addFile(wiz.sep('Pruebas Anteriores'), '', icon=ICONMAINT, themeit=THEME3)
            for item in speedimg:
                ...
                addFile('[B]%s[/B]: [I]Resultado %s[/I]' % (img, created), 'viewspeedtest', img, icon=ICONMAINT, themeit=THEME3)
...
def runSpeedTest():
    try:
        ...
        wiz.log("[Speed Test] Error al Iniciar Prueba de Velocidad")
        pass
...
def apkInstaller(apk, url):
    ...
    else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: No es un Dispositivo Android[/COLOR]' % COLOR2)

def romInstaller(name, url):
    myroms = xbmc.translatePath(BACKUPROMS)
    if myroms == '':
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Parece que no hay configurada una ruta para extraer los archivos" % COLOR2, "Quieres configurar una ahora?[/COLOR]", yeslabel="[COLOR springgreen][B]Aceptar[/B][/COLOR]", nolabel="[COLOR red][B]Cancelar[/B][/COLOR]"):
            wiz.openS()
            myroms = wiz.getS('rompath')
            if myroms == '': return
    yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres Descargar y Extraer [COLOR %s]%s[/COLOR] en:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR springgreen]Aceptar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
    if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Instalacion Cancelada[/COLOR]' % COLOR2); return
    display = name
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Instalador de APKs: Url Rom incorrecta![/COLOR]' % COLOR2); return
    DP.create(ADDONTITLE,'[COLOR %s][B]Descargando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Espera por favor')
    ...
    wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Pack de Rom Instalado[/COLOR]' % COLOR2)
    DP.close()
...
def createMenu(type, add, name):
    if   type == 'saveaddon':
        ...
        menu_items.append((THEME3 % 'Guardar %s Datos' % add3,           'RunPlugin(plugin://%s/?mode=save%s&name=%s)' %    (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Restaurar %s Datos' % add3,         'RunPlugin(plugin://%s/?mode=restore%s&name=%s)' % (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Limpiar %s Datos' % add3,           'RunPlugin(plugin://%s/?mode=clear%s&name=%s)' %   (ADDON_ID, add2, name2)))
    elif type == 'save'    :
        menu_items=[]
        add2  = urllib.quote_plus(add.lower().replace(' ', ''))
        add3  = add.replace('Debrid', 'Real Debrid')
        ...
        name = name.replace('url', 'URL Resolver')
        menu_items.append((THEME2 % name.title(),             ' '))
        menu_items.append((THEME3 % 'Registro %s' % add3,                'RunPlugin(plugin://%s/?mode=auth%s&name=%s)' %    (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Guardar %s Datos' % add3,           'RunPlugin(plugin://%s/?mode=save%s&name=%s)' %    (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Restaurar %s Datos' % add3,         'RunPlugin(plugin://%s/?mode=restore%s&name=%s)' % (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Importar %s Datos' % add3,          'RunPlugin(plugin://%s/?mode=import%s&name=%s)' %  (ADDON_ID, add2, name2)))
        menu_items.append((THEME3 % 'Limpiar Addon %s Datos' % add3,     'RunPlugin(plugin://%s/?mode=addon%s&name=%s)' %   (ADDON_ID, add2, name2)))
    elif type == 'install'  :
        menu_items=[]
        name2 = urllib.quote_plus(name)
        menu_items.append((THEME2 % name,                                'RunAddon(%s, ?mode=viewbuild&name=%s)'  % (ADDON_ID, name2)))
        menu_items.append((THEME3 % 'Instalacion Limpia',                'RunPlugin(plugin://%s/?mode=install&name=%s&url=fresh)'  % (ADDON_ID, name2)))
        menu_items.append((THEME3 % 'Instalacion Normal',                'RunPlugin(plugin://%s/?mode=install&name=%s&url=normal)' % (ADDON_ID, name2)))
        menu_items.append((THEME3 % 'Aplicar guiFix',                    'RunPlugin(plugin://%s/?mode=install&name=%s&url=gui)'    % (ADDON_ID, name2)))
        menu_items.append((THEME3 % 'Informacion de Build',              'RunPlugin(plugin://%s/?mode=buildinfo&name=%s)'  % (ADDON_ID, name2)))
    ...
    if state in ['true', 'false']:
        ...
        if not state in ['includevideo', 'includeall'] and wiz.getS('includeall') == 'true':
            try:
                ...
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Tienes que desactivar [COLOR %s]Incluir Todos los Addons[/COLOR] para poder desactivar[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, item))
            except:
                wiz.LogNotify("[COLOR %s]Alternar Cache[/COLOR]" % COLOR1, "[COLOR %s]ID incorrecta: %s[/COLOR]" % (COLOR2, state))
        else:
...
def viewLogFile():
    ...
    if not oldlog == False and not mainlog == False:
        ...
        if which == -1: wiz.LogNotify('[COLOR %s]Ver Registro[/COLOR]' % COLOR1, '[COLOR %s]Ver Registro Cancelado![/COLOR]' % COLOR2); return
    elif mainlog == False and oldlog == False:
        wiz.LogNotify('[COLOR %s]Ver Registro[/COLOR]' % COLOR1, '[COLOR %s]No se ha encontrado Archivo de Registro![/COLOR]' % COLOR2)
        return
...
def errorList(file):
    ...
    a=open(file).read()
    b=a.replace('\n','[CR]').replace('\r','')
    match = re.compile("-->Python devolvio el siguiente error<--(.+?)-->Fin del Informe de Error de Python<--").findall(b)
    for item in match:
        errors.append(item)
    return errors
...
def errorChecking(log=None, count=None, last=None):
    ...
    if log == None:
        ...
        if old == False and curr == False:
            if count == None:
                wiz.LogNotify('[COLOR %s]Ver Archivo de Registro[/COLOR]' % COLOR1, '[COLOR %s]No se ha encontrado Archivo de Registro![/COLOR]' % COLOR2)
                return
            else:
                return 0
        if not curr == False:
    ...
    if not count == None:
        ...
        if last == None:
            ...
            for item in errors:
                i += 1
                string += "[B][COLOR red]ERROR NUMERO %s:[/B][/COLOR]%s\n" % (str(i), item.replace(HOME, '/').replace('                                        ', ''))
        else:
            string = "[B][COLOR red]Ultimo Error en Registro:[/B][/COLOR]%s\n" % (errors[0].replace(HOME, '/').replace('                                        ', ''))
        wiz.TextBox("%s: Errores en Registro" % ADDONTITLE, string)
    else:
        wiz.LogNotify('[COLOR %s]Ver Registro de Errores[/COLOR]' % COLOR1, '[COLOR %s]No se encontraron Errores![/COLOR]' % COLOR2)
...
def removeAddon(addon, name, over=False):
    if not over == False:
        ...
        yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres Borrar el addon:'% COLOR2, 'Nombre: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR springgreen]Borrar[/COLOR][/B]', nolabel='[B][COLOR red]No Borrar[/COLOR][/B]')
    if yes == 1:
    ...
    if over == False:
        wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Borrados[/COLOR]" % (COLOR2, name))

def removeAddonData(addon, name=None, over=False):
    if addon == 'all':
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar [COLOR %s]TODOS[/COLOR] los datos de la carpeta de datos de usuario?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
            wiz.cleanHouse(ADDOND)
        else: wiz.LogNotify('[COLOR %s]Borrar Datos de Addon[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
    elif addon == 'uninstalled':
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres Borrar [COLOR %s]TODOS[/COLOR] los datos almacenados en tu carpeta de datos de usuario para los addons desinstalados?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
            total = 0
            ...
            wiz.LogNotify('[COLOR %s]Limpieza Desinstalada[/COLOR]' % COLOR1, '[COLOR %s]%s Carpeta(s) Eliminada(s)[/COLOR]' % (COLOR2, total))
        else: wiz.LogNotify('[COLOR %s]Borrar Datos de Addon[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
    elif addon == 'empty':
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODAS[/COLOR] las carpetas vacias de tu Carpeta Userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]', nolabel='[B][COLOR red]Cancelar[/COLOR][/B]'):
            total = wiz.emptyfolder(ADDOND)
            wiz.LogNotify('[COLOR %s]Borrar Carpetas Vacias[/COLOR]' % COLOR1, '[COLOR %s]%s Carpeta(s) Cancelada(s)[/COLOR]' % (COLOR2, total))
        ...
        if addon in EXCLUDES:
            wiz.LogNotify("[COLOR %s]Plugin Protegido[/COLOR]" % COLOR1, "[COLOR %s]No permite borrar datos[/COLOR]" % COLOR2)
        elif os.path.exists(addon_data):
            if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar los datos del addon:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR springgreen]Borrar[/COLOR][/B]', nolabel='[B][COLOR red]No Borrar[/COLOR][/B]'):
                wiz.cleanHouse(addon_data)
                try:
                    shutil.rmtree(addon_data)
                except:
                    wiz.log("Error al borrar: %s" % addon_data)
            else:
                wiz.log('Datos de Addon para %s no fueron eliminados' % addon)
    wiz.refresh()
...
def restoreit(type):
    ...
    if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
        wiz.skinToDefault('Restaurar Copia de Seguridad')
    ...
    if wiz.workingURL(BUILDFILE) == True:
        if wiz.checkBuild(name, 'url'):
            ...
            if extend == True:
                ...
                msg += "[COLOR %s]Version de Kodi:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, kodi)
        ...
        else: wiz.log("Nombre de Build incorrecto!")
    else: wiz.log("Build.txt no funciona: %s" % WORKINGURL)

def buildVideo(name):
    if wiz.workingURL(BUILDFILE) == True:
        videofile = wiz.checkBuild(name, 'preview')
        if videofile and not videofile == 'http://': playVideo(videofile)
        else: wiz.log("[%s]No se puede encontrar la URL para la vista previa de video" % name)
    else: wiz.log("Build.txt no funciona: %s" % WORKINGURL)
...
def manageSaveData(do):
    if do == 'import':
        ...
        source = DIALOG.browse(1, '[COLOR %s]Selecciona la ubicacion de SaveData.zip[/COLOR]' % COLOR2, 'files', '.zip', False, False, HOME)
        ...
        if extract.all(xbmc.translatePath(tempfile), TEMP) == False:
            wiz.log("Error al intentar extraer el archivo zip!")
            wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error al importar datos![/COLOR]" % (COLOR2))
            return
            ...
            for item in files:
                ...
                if os.path.exists(old):
                    if overwrite == 1: os.remove(old)
                        ...
                        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres reemplazar el actual [COLOR %s]%s[/COLOR] archivo?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR springgreen]Reemplazar[/COLOR][/B]", nolabel="[B][COLOR red]Saltar[/COLOR][/B]"): continue
                        else: os.remove(old)
        ...
        if os.path.exists(super):
            ...
            if os.path.exists(old):
                cont = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres reemplazar la carpeta actual de [COLOR %s]Super Favourites[/COLOR] addon_data por la nueva?" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Reemplazar[/COLOR][/B]", nolabel="[B][COLOR red]Saltar[/COLOR][/B]")
            else: cont = 1
            if cont == 1:
                wiz.cleanHouse(old)
        ...
        if x == 0: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error al Importar Datos Guardados[/COLOR]" % COLOR2)
        else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importar Datos Guardados Completo[/COLOR]" % COLOR2)
    elif do == 'export':
        ...
        source = DIALOG.browse(3, '[COLOR %s]Seleccione donde desea exportar el zip de los datos guardados?[/COLOR]' % COLOR2, 'archivos', '', False, True, HOME)
        source = xbmc.translatePath(source)
        ...
        zipf.close()
        if source == mybuilds:
            DIALOG.ok(ADDONTITLE, "[COLOR %s]Se ha realizado una copia de seguridad de los datos guardados en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
        else:
            try:
                xbmcvfs.copy(tempzip, os.path.join(source, 'SaveData.zip'))
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Se ha realizado una copia de seguridad de los datos guardados en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'SaveData.zip')))
            except:
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Se ha realizado una copia de seguridad de los datos guardados en:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
...
def freshStart(install=None, over=False):
    ...
    elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restaurar tu" % COLOR2, "Configuracion de Kodi a sus Ajustes por defecto", "Antes de instalar la copia de seguridad?[/COLOR]", nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restaurar tu" % COLOR2, "Configuracion de Kodi a sus Ajustes por defecto", "Antes de instalar [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restaurar tu" % COLOR2, "Configuracion de Kodi a sus Ajustes por defecto?[/COLOR]", nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    if yes_pressed:
        if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
            swap = wiz.skinToDefault('Instalacion Nueva')
            if swap == False: return False
            xbmc.sleep(1000)
        if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
            wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Instalacion Nueva: Fallo en el cambio de skin![/COLOR]' % COLOR2)
            return
        wiz.addonUpdates('set')
        xbmcPath=os.path.abspath(HOME)
        DP.create(ADDONTITLE,"[COLOR %s]Calculando archivos y carpetas" % COLOR2,'', 'Please Wait![/COLOR]')
        ...
        DP.update(0, "[COLOR %s]Borrando archivos y carpetas:" % COLOR2)
        ...
        for root, dirs, files in os.walk(xbmcPath,topdown=True):
            ...
            for name in files:
                ...
                if name == 'sources.xml' and fold[-1] == 'userdata' and KEEPSOURCES == 'true': wiz.log("Guardar Fuentes: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
                ...
                elif name == 'profiles.xml' and fold[-1] == 'userdata' and KEEPPROFILES == 'true': wiz.log("Guardar Perfiles: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
                elif name == 'advancedsettings.xml' and fold[-1] == 'userdata' and KEEPADVANCED == 'true':  wiz.log("Guardar Ajustes Avanzados: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
                elif name in LOGFILES: wiz.log("Guardar Archivo Log: %s" % name, xbmc.LOGNOTICE)
                elif name.endswith('.db'):
                    try:
                        if name == latestAddonDB and KODIV >= 17: wiz.log("Aplazando %s en v%s" % (name, KODIV), xbmc.LOGNOTICE)
                        else: os.remove(os.path.join(root,name))
                    except Exception, e:
                        if not name.startswith('Textures13'):
                            wiz.log('Fallo al borrar, Eliminando DB', xbmc.LOGNOTICE)
                    ...
                    try: os.remove(os.path.join(root,name))
                        ...
                        wiz.log("Error al borrar %s" % os.path.join(root, name), xbmc.LOGNOTICE)
                        wiz.log("-> / %s" % (str(e)), xbmc.LOGNOTICE)
            if DP.iscanceled():
                DP.close()
                wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Instalacion Limpia Cancelada[/COLOR]" % COLOR2)
                return False
        for root, dirs, files in os.walk(xbmcPath,topdown=True):
            dirs[:] = [d for d in dirs if d not in EXCLUDES]
            for name in dirs:
                DP.update(100, '', 'Limpiando Carpeta Vacia: [COLOR %s]%s[/COLOR]' % (COLOR1, name), '')
        ...
        if over == True:
            ...
            else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el Cierre[/COLOR] de kodi o [COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR springgreen]Forzar Cierre[/COLOR][/B]")
...
####THANKS GUYS @ NaN #######
def clearCache():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]'):
        wiz.clearCache()

def clearArchive():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar la carpeta \'Archive_Cache\'?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]'):
        wiz.clearArchive()

def totalClean():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar cache, paquetes y miniaturas?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Aceptar[/COLOR][/B]'):
        wiz.clearCache()
...
def clearThumb(type=None):
    ...
    else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar %s y las carpetas de miniaturas relacionadas?' % (COLOR2, latest), "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR red]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Delete Thumbs[/COLOR][/B]')
    if choice == 1:
        try: wiz.removeFile(os.join(DATABASE, latest))
        except: wiz.log('Error al Eliminar, Borrando Base de Datos.'); wiz.purgeDb(latest)
        for i in thumb_locations:
            wiz.removeFolder(i)
        #if not type == 'total': wiz.killxbmc()
    else: wiz.log('Borrar Nombres Cancelado')
    wiz.redoThumbs()
...
def purgeDb():
    ...
    if KODIV >= 16:
        choice = DIALOG.multiselect("[COLOR %s]Selecciona Base de Datos que quieres Borrar[/COLOR]" % COLOR2, display)
        if choice == None: wiz.LogNotify("[COLOR %s]Borrado de Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
        elif len(choice) == 0: wiz.LogNotify("[COLOR %s]Borrado de Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
        else:
            for purge in choice: wiz.purgeDb(DB[purge])
    else:
        choice = DIALOG.select("[COLOR %s]Selecciona Base de Datos que quieres Borrar[/COLOR]" % COLOR2, display)
        if choice == -1: wiz.LogNotify("[COLOR %s]Borrado de Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
...
def testnotify():
    ...
    if url == True:
        try:
            ...
            if id == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Notificacion: No Formateado correctamente[/COLOR]" % COLOR2); return
            notify.notification(msg, True)
        except Exception, e:
            wiz.log("Error en Ventana de Notificaciones: %s" % str(e), xbmc.LOGERROR)
    else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]URL incorrecta para la Notificacion[/COLOR]" % COLOR2)
